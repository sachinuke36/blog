...
const BlogCard2 = ({blog}:{blog: Blog}) => {
  ...
  return (
     <Card  onClick={()=> router.push(`/blog/${blog?.id}`)} className='flex flex-col h-[250px]  w-[350px] border-none text-white shadow shadow-orange-300 gap-3 bg-[#353535ee]' >
            ...
            <div className='px-2  flex gap-2'>{blog.tags.filter((tag, i) => i < 3).map((tag, i) => <p key={i} className='text-[10px] flex items-center justify-center shadow-purple-300 shadow-sm  bg-[#121111] px-1'>{tag}</p>)}</div>
